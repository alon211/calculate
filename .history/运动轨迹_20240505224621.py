...
a= np.multiply(Jerk, x_values)
v= np.multiply(a, x_values)
s= np.multiply(v, x_values)
...
a_formula = sp.integrate(Jerk, x)
print(f'a(t)={a_formula}')
v_formula = sp.integrate(a, x)
print(f'v(t)={v_formula}')
s_formula = sp.integrate(v, x)
...
plt.plot(x_values, v, label=f'$s(x) = {s_formula}$', color='black')
