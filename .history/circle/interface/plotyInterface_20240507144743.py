...
import numpy as np
import functools
__all__=['radius','ex_radius','center_x','center_y','ani','init','update']
...
# 初始化函数，设定绘图元素
def init(ex_radius,center_x, center_y):
    ax.set_xlim(0, (ex_radius+center_x)+0.1)
    ax.set_ylim(0, (ex_radius+center_y)+0.1)
    point.set_data([], [])
...
# 更新函数，用于动画的每一帧
def update(frame,radius,center_x, center_y):
    # 移动到圆的边缘
    ...
    return point, trajectory_line
def module_init(radius,ex_radius,center_x, center_y):   
