...
init_partial = functools.partial(init, ax=ax,ani=ani,tool_r=tool_r/2.0,point=point,trajectory_line=trajectory_line,ex_radius=ex_radius,center_x=center_x, center_y=center_y)
update_partial = functools.partial(update,max_frames=max_frames,accurancy=accurancy, point=point, trajectory_line=trajectory_line,radius=act_radius,center_x=center_x, center_y=center_y)
# 创建动画对象
ani = FuncAnimation(fig, update_partial, frames=np.arange(0, max_frames, 1),
                    init_func=init_partial, blit=True,interval=10)
