...
def update(frame):
    ...
    if frame*accurancy < act_radius+act_radius*act_n :   #直线运动 frame从0开始所以小一个accurancy单位切换
        action=LINE
        
        
    if frame*accurancy == act_radius+act_radius*act_n:
        act_n=act_n+1
        action=CIRCLE
    if frame*accurancy >= act_radius+act_radius*act_n and frame<act_radius+act_radius*(act_n+1): #圆运动
        action=CIRCLE
